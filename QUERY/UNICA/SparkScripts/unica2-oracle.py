...
conf = SparkConf().setMaster("spark://spark-master:7077").setAppName("unica2-oracle").set("spark.executor.memory","8G").set("spark.driver.memory","8G").set("spark.sql.inMemoryColumnarStorage.batchSize","100000")
...
tab4 = sqlContext.read.format("jdbc").options(url="jdbc:oracle:thin:esse3/esse3@//spark-db:1521/orcl.unica.it",dbtable="P02_RISPOSTE",fetchSize=10000, partitionColumn="QUESITO_ID" , lowerBound=1, upperBound=127341, numPartitions=16).load()
...
tab6 = sqlContext.read.format("jdbc").options(url="jdbc:oracle:thin:esse3/esse3@//spark-db:1521/orcl.unica.it",dbtable="V02_RISPOSTE_ROW_TESTO_LIBERO",fetchSize=10000, partitionColumn="QUESITO_ID" , lowerBound=1, upperBound=127341, numPartitions=16).load()
...
tab8 = sqlContext.read.format("jdbc").options(url="jdbc:oracle:thin:esse3/esse3@//spark-db:1521/orcl.unica.it",dbtable="Q35_DATI_COMP",fetchSize=100000, partitionColumn="AA_OFF_AD_ID_VAL " , lowerBound=2013, upperBound=2016, numPartitions=5).load()
