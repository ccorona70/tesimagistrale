...
#conf =SparkConf().setMaster("local").setAppName("Ontime_oracle_q1").set("spark.executor.memory”,”4G").set("spark.driver.memory","4G")

sc =SparkContext(conf =conf)

sqlContext =SQLContext(sc)

tab1 =sqlContext.read.format("jdbc").options(url="jdbc:oracle:thin:esse3/esse3@//localhost:1521/orcl.unica.it",dbtable="ontime", fetchSize=10000, partitionColumn="year" , lowerBound=2007, upperBound=2017, numPartitions=11).load()
